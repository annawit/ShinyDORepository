...
load("//deqhq1/TMDL/PSU TMDL Status and Trend Study/North Coast dissolved oxygen - Anna Withington/Scripts and R data files/ShinyappDOdatamerge/ShinyAllData.Rdata")
load("//deqhq1/TMDL/PSU TMDL Status and Trend Study/North Coast dissolved oxygen - Anna Withington/Scripts and R data files/ShinyappDOdatamerge/PracticeDataLocationInfo.Rdata")
                                  ...
                                  sidebarLayout(
                                    sidebarPanel(
                                      ...
                                      checkboxGroupButtons(
                                        ...
                                        selected = c("lasar_id", "datetime", "temp", "ph", "do",
                                                     "cond", "data_source", "MonitoringLocationName")
                                      ),
                                      HTML("Select a station and variables, then hit 'Download data'."),
                                      br(), br(),
                                      downloadButton(
...
server = function(input, output, session) {
  ...
  output$download_data <- downloadHandler(
    ...
    #   function() {
    #   paste0("ShinyDataTableDownload.", csv)
    # }
  ...
  output$summaryboxplot <- renderPlotly({
    boxplotData <- mapData %>% 
      ...
      mutate(season = factor(month.abb[month(datetime)],
                             levels = c("May", "Jun", "Jul", "Aug", "Oct")))
    plot_ly(boxplotData, x = ~lasar_id, y = ~do, color = ~season,
